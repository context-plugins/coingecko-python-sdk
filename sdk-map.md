...
| `header_auth` | `str \| None` | `str \| None` | `None` |
| `query_auth` | `str \| None` | `str \| None` | `None` |
...
**Raw (`.with_raw_response`) variants: present on every operation** — the same call returns `ApiResult` instead of raising, with the same body on `Failure.error`. Of **61 operations**, **0 are Case A (typed)** and **61 are Case B (raw)**.

---

## Operations — by controller (11 pages, 61 operations)
...
| Controller | Ops | Page |
| --- | --- | --- |
| `client.coins` | 13 | [map/operations/coins.md](map/operations/coins.md) |
| `client.derivatives` | 4 | [map/operations/derivatives.md](map/operations/derivatives.md) |
| `client.entities` | 2 | [map/operations/entities.md](map/operations/entities.md) |
| `client.exchanges` | 6 | [map/operations/exchanges.md](map/operations/exchanges.md) |
| `client.global_api` | 2 | [map/operations/global_api.md](map/operations/global_api.md) |
| `client.misc` | 3 | [map/operations/misc.md](map/operations/misc.md) |
| `client.nfts` | 3 | [map/operations/nfts.md](map/operations/nfts.md) |
| `client.onchain` | 20 | [map/operations/onchain.md](map/operations/onchain.md) |
| `client.public_treasury_api` | 3 | [map/operations/public_treasury_api.md](map/operations/public_treasury_api.md) |
| `client.search_api` | 2 | [map/operations/search_api.md](map/operations/search_api.md) |
| `client.simple` | 3 | [map/operations/simple.md](map/operations/simple.md) |
...
Every model, enum and union also has an **input companion**, exported beside it from the same package (`AssetPlatform` ↔ `AssetPlatformDict`). Wherever a signature names the companion you may pass either the model instance or a plain dict with the same keys, whichever reads better at the call site. An enum is a real `Enum` subclass over `str`; its companion is spelled `<Name>OrStr` or `<Name>OrInt` (`Type` ↔ `TypeOrStr`) and additionally accepts a wire value this SDK version does not know. A union is a `TypeAlias` over its arms.
...
**API key (header `x-cg-demo-api-key`).** Pass `header_auth="<api_key>"`; sent as the `x-cg-demo-api-key` request header.

**API key (query `x_cg_demo_api_key`).** Pass `query_auth="<api_key>"`; sent as the `x_cg_demo_api_key` query parameter.
...
| --- | --- |
| `https://api.coingecko.com/api/v3` | `base_url="https://…"` |
